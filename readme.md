...
```
📦 elyfaUniverse
├── api.py                          # FastAPI-сервис: эндпоинты /predict, /predict_batch, /predict_csv
├── app.py                          # Streamlit-приложение: интерфейс для предсказания одного авто и загрузки CSV
├── model.pkl                       # Обученная модель Ridge (alpha=1000) – сериализована joblib
├── scaler.pkl                      # StandardScaler, обученный на тренировочных данных
├── feature_cols.pkl                # Список признаков в порядке, ожидаемом моделью
├── requirements.txt                # Зависимости Python (FastAPI, uvicorn, streamlit, pandas, sklearn и др.)
├── homework_1_part_2_regression_with_inference.ipynb  # Ноутбук с полным пайплайном: EDA, предобработка, обучение, сохранение артефактов
├── test_cars.csv                   # Тестовая выборка (1000 объектов) для проверки модели
├── data_for_plot.json              # Данные для построения графика в Streamlit (опционально)
├── ydata_report.html               # Интерактивный профиль данных (сгенерирован ydata-profiling)
├── ydata_report.png                # Скриншот / визуализация отчёта профиля данных
├── README.md                       # Основной README проекта (описание, ссылки, инструкции)
├── readme.md                       # Дополнительная версия / черновик README
└── LICENSE                         # Лицензия проекта
```

### Что за что отвечает

- **api.py** – бэкенд-сервис на FastAPI. Принимает JSON одного автомобиля или CSV-файл, возвращает предсказанную цену. Валидация входных данных через Pydantic (`CarData`).
- **app.py** – фронтенд на Streamlit. Позволяет пользователю ввести характеристики автомобиля в веб-форме или загрузить CSV и сразу получить предсказания.
- **model.pkl**, **scaler.pkl**, **feature_cols.pkl** – три обязательных артефакта, которые загружаются и в `api.py`, и в `app.py`. Без них сервисы не запустятся.
- **requirements.txt** – список всех библиотек, необходимых для запуска сервисов. Установка одной командой: `pip install -r requirements.txt`.
- **homework_1_part_2_regression_with_inference.ipynb** – исходный Colab-ноутбук с полным циклом: от загрузки данных до экспорта артефактов. Содержит все эксперименты и выводы.
- **test_cars.csv** – тестовый датасет, используется для демонстрации загрузки в Streamlit и проверки `/predict_csv`.
- **data_for_plot.json** – дополнительные данные, вероятно для визуализации взаимосвязей признаков прямо в интерфейсе Streamlit.
- **ydata_report.html** и **ydata_report.png** – статический профиль данных, созданный на этапе EDA для быстрого понимания структуры датасета.
- **README.md** – главный файл документации, который отображается на GitHub. Обычно содержит краткое описание, ссылки на видео и инструкции по запуску.
- **LICENSE** – лицензия на использование и распространение кода.
```
