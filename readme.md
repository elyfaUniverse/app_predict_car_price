 ...
 **Артефакты модели** | |
| `model.pkl` | Обученная Ridge-модель  |
 ...
 **Аналитика и отчёты** | |
| `homework_1_part_2_regression_with_inference.ipynb` | Полный ноутбук EDA |
| `ydata_report.html` | Интерактивный профиль данных ydata-profiling |
...
### ✅ 1. Анализ данных
Я загрузила тренировочный `cars_train.csv` и тестовый `cars_test.csv` датасеты. Посмотрела на данные с помощью обычных описательных статистик и быстрого профиля через `ydata-profiling`.  
**Главные наблюдения:**
...
- Топливо почти пополам: дизель и бензин, газ редко.
- Продавцы в основном частные, коробка передач ручная.
- Пропуски были в `mileage`, `engine`, `max_power`, `torque`, `seats`. Дубликатов не нашлось.
- Числовые признаки `mileage`, `engine`, `max_power`, `torque` записаны как текст с единицами измерения.

Я построила корреляционные матрицы.  
- **Сильнее всего с ценой связаны:** `max_power`, `torque`, `engine`, `year`.  
- Пробег и расход топлива почти не влияют на цену.  
- Между годом выпуска и пробегом корреляция -0.37 .  
- `max_power` и `torque` сильно связаны друг с другом.
---
...
**Пропуски** заполнила медианой, посчитанной **только на тренировочных данных**. Медиана не боится выбросов, и из-за малого количества пропусков распределения совсем не сдвинулись.
...
Пробовала удалять машины с аномально высокой ценой R² резко упал до 0.49, значит, выбросы несли полезную информацию. Логарифмирование цены сделало распределение красивее, но точность модели не поднялась.
...
- Описан класс `CarData` наследуется от `BaseModel` модель для входных данных одного автомобиля.
...
После этого сервис будет доступен по адресу `http://localhost:8000`, а документация с возможностью тестирования доступна на `http://localhost:8000/docs`.
...
### Моё приложение живёт здесь: Streamlit нажми на кнопку 
👉👉👉[![Streamlit App](https://img.shields.io/badge/Streamlit-FF4B4B?style=for-the-badge&logo=Streamlit&logoColor=white)](https://apppredictcarprice.streamlit.app/)
...
### Лучшая модель
Я сравнила несколько вариантов, и для деплоя выбрала **Ridge с α=1000**, потому что она дала самую высокую долю прогнозов с ошибкой ≤10%.
...
Мощность двигателя это самый главный фактор цены, за ней идут год выпуска и крутящий момент. Расход топлива и объём двигателя почти не играют роли.

---

## Что дало наибольший прирост качества

- **Преобразование текстовых полей с единицами в числа**.
- **Восемь исходных числовых признаков**.
- **Отказ от логарифмирования цены**.
- **Ridge с небольшим α**.
...
Я построила полный пайплайн: от анализа и очистки данных до двух работающих сервисов  FastAPI  и Streamlit. Модель объясняет около 60% разброса цен и даёт приемлемую точность для каждого четвёртого автомобиля. Главные факторы стоимости это мощность, год выпуска и крутящий момент.
