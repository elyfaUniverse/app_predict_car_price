...
Я построила полный пайплайн: от анализа и очистки данных до работающего сервиса на FastAPI с двумя основными эндпоинтами (один объект и CSV). Модель объясняет около 57–60% разброса цен и даёт приемлемую точность для каждого четвёртого автомобиля. Главные факторы стоимости — мощность, год выпуска и крутящий момент.
